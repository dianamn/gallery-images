...
$GLOBALS['$hugeit_gallery_aliases'] = array(
	...
	'Gallery_Img_Galleries'        => 'includes/class-gallery-img-galleries',
...
function gallery_img_autoload( $classname ) {
	...
	if ( ! key_exists( $classname, $hugeit_gallery_aliases ) ) {
		throw new Exception( 'trying to load "' . $classname . '" class that is not registered in config file.' );
	}

	$path = Gallery_Img()->plugin_path() . '/' . $hugeit_gallery_aliases[ $classname ] . '.php';
